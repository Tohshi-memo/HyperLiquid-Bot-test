# Latest Canary Signals

These are early-warning indicators for cross-market relationships. They are hypotheses to test, not trade signals by themselves.

- Updated: `2026-07-10T12:07:30.293679+00:00`
- Correlation status: `ready`
- Asset price records: `672`
- Minimum samples for correlation: `24`

## Current Signals

- baseline: score `0` - No elevated canary signal. Continue collecting samples.

## Class Returns

- 15m: commodity avg `-0.0308` n `12`; crypto_alt avg `-0.1092` n `229`; crypto_major avg `-0.2234` n `8`; equity avg `-0.1779` n `91`; fx avg `-0.0063` n `6`; index avg `-0.0415` n `25`; metal avg `-0.0553` n `20`; unknown avg `0.0286` n `766`
- 1h: commodity avg `0.0886` n `12`; crypto_alt avg `-0.217` n `229`; crypto_major avg `-0.2147` n `8`; equity avg `-0.2897` n `91`; fx avg `0.0016` n `6`; index avg `-0.0743` n `25`; metal avg `0.0043` n `20`; unknown avg `-0.0129` n `766`
- 4h: commodity avg `0.3184` n `12`; crypto_alt avg `0.3583` n `229`; crypto_major avg `0.4091` n `8`; equity avg `0.5532` n `91`; fx avg `0.0156` n `6`; index avg `0.0425` n `25`; metal avg `-0.0743` n `20`; unknown avg `0.0581` n `765`
- 24h: commodity avg `-0.9008` n `12`; crypto_alt avg `1.1109` n `229`; crypto_major avg `1.7152` n `8`; equity avg `0.3048` n `91`; fx avg `-0.1058` n `6`; index avg `0.1094` n `25`; metal avg `-0.0209` n `20`; unknown avg `0.1227` n `733`

## Correlations

- news_risk_score -> fx_forward_1h_return_pct: corr `0.1107`, n `668`, weak_sample_signal
- polymarket_volume_24h -> metal_forward_1h_return_pct: corr `-0.1083`, n `668`, weak_sample_signal
- flow_alert_score -> index_forward_1h_return_pct: corr `-0.096`, n `668`, weak_sample_signal
- risk_on_score -> index_forward_1h_return_pct: corr `0.0926`, n `668`, weak_sample_signal
- risk_on_score -> metal_forward_1h_return_pct: corr `0.0898`, n `668`, weak_sample_signal
- polymarket_volume_24h -> index_forward_1h_return_pct: corr `-0.0868`, n `668`, weak_sample_signal
- risk_on_score -> crypto_alt_forward_1h_return_pct: corr `0.0854`, n `668`, weak_sample_signal
- polymarket_volume_24h -> equity_forward_1h_return_pct: corr `-0.0762`, n `668`, weak_sample_signal
- flow_alert_score -> equity_forward_1h_return_pct: corr `-0.0742`, n `668`, weak_sample_signal
- market_context_score -> unknown_forward_1h_return_pct: corr `0.0724`, n `668`, weak_sample_signal
