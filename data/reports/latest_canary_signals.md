# Latest Canary Signals

These are early-warning indicators for cross-market relationships. They are hypotheses to test, not trade signals by themselves.

- Updated: `2026-08-01T02:52:27.611304+00:00`
- Correlation status: `ready`
- Asset price records: `672`
- Minimum samples for correlation: `24`

## Current Signals

- baseline: score `0` - No elevated canary signal. Continue collecting samples.

## Class Returns

- 15m: commodity avg `-0.0482` n `12`; crypto_alt avg `0.0602` n `230`; crypto_major avg `-0.0493` n `8`; equity avg `0.0114` n `102`; fx avg `-0.0015` n `6`; index avg `0.0054` n `25`; metal avg `0.0009` n `20`; unknown avg `-0.066` n `781`
- 1h: commodity avg `-0.1104` n `12`; crypto_alt avg `0.0984` n `230`; crypto_major avg `0.0461` n `8`; equity avg `0.0818` n `102`; fx avg `-0.0049` n `6`; index avg `-0.0196` n `25`; metal avg `-0.0057` n `20`; unknown avg `0.1561` n `781`
- 4h: commodity avg `-0.1433` n `12`; crypto_alt avg `0.699` n `230`; crypto_major avg `0.2301` n `8`; equity avg `0.1254` n `102`; fx avg `-0.0339` n `6`; index avg `0.0266` n `25`; metal avg `-0.0286` n `20`; unknown avg `4.0399` n `781`
- 24h: commodity avg `0.8856` n `12`; crypto_alt avg `0.3484` n `230`; crypto_major avg `-1.3597` n `8`; equity avg `-1.947` n `102`; fx avg `-0.1234` n `6`; index avg `-0.1704` n `25`; metal avg `-0.1596` n `20`; unknown avg `4.9194` n `747`

## Correlations

- flow_alert_score -> commodity_forward_1h_return_pct: corr `0.1058`, n `668`, weak_sample_signal
- polymarket_volume_24h -> crypto_alt_forward_1h_return_pct: corr `-0.1036`, n `668`, weak_sample_signal
- polymarket_volume_24h -> commodity_forward_1h_return_pct: corr `0.1031`, n `668`, weak_sample_signal
- risk_on_score -> crypto_major_forward_1h_return_pct: corr `-0.0874`, n `668`, weak_sample_signal
- flow_alert_score -> crypto_alt_forward_1h_return_pct: corr `-0.0749`, n `668`, weak_sample_signal
- news_risk_score -> commodity_forward_1h_return_pct: corr `-0.0739`, n `668`, weak_sample_signal
- polymarket_volume_24h -> crypto_major_forward_1h_return_pct: corr `-0.0702`, n `668`, weak_sample_signal
- news_risk_score -> index_forward_1h_return_pct: corr `0.0694`, n `668`, weak_sample_signal
- risk_on_score -> metal_forward_1h_return_pct: corr `0.0688`, n `668`, weak_sample_signal
- news_risk_score -> equity_forward_1h_return_pct: corr `0.0644`, n `668`, weak_sample_signal
