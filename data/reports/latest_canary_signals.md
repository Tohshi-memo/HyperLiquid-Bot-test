# Latest Canary Signals

These are early-warning indicators for cross-market relationships. They are hypotheses to test, not trade signals by themselves.

- Updated: `2026-08-26T07:37:25.579356+00:00`
- Correlation status: `ready`
- Asset price records: `672`
- Minimum samples for correlation: `24`

## Current Signals

- baseline: score `0` - No elevated canary signal. Continue collecting samples.

## Class Returns

- 15m: commodity avg `-0.0278` n `12`; crypto_alt avg `-0.1794` n `231`; crypto_major avg `-0.0865` n `8`; equity avg `-0.0738` n `122`; fx avg `0.0027` n `6`; index avg `-0.027` n `25`; metal avg `0.0068` n `20`; unknown avg `0.0538` n `797`
- 1h: commodity avg `-0.1284` n `12`; crypto_alt avg `0.1661` n `231`; crypto_major avg `0.3159` n `8`; equity avg `-0.1536` n `122`; fx avg `0.0078` n `6`; index avg `-0.0415` n `25`; metal avg `0.0414` n `20`; unknown avg `-0.0573` n `797`
- 4h: commodity avg `0.025` n `12`; crypto_alt avg `-0.0956` n `231`; crypto_major avg `0.0587` n `8`; equity avg `-0.3016` n `122`; fx avg `-0.0111` n `6`; index avg `-0.0452` n `25`; metal avg `-0.1216` n `20`; unknown avg `0.1197` n `781`
- 24h: commodity avg `-0.6458` n `12`; crypto_alt avg `-2.5746` n `231`; crypto_major avg `-2.4451` n `8`; equity avg `0.5379` n `122`; fx avg `-0.0369` n `6`; index avg `0.0586` n `25`; metal avg `0.1715` n `20`; unknown avg `0.7066` n `778`

## Correlations

- flow_alert_score -> crypto_major_forward_1h_return_pct: corr `0.186`, n `668`, weak_sample_signal
- flow_alert_score -> crypto_alt_forward_1h_return_pct: corr `0.1417`, n `668`, weak_sample_signal
- flow_alert_score -> metal_forward_1h_return_pct: corr `0.1309`, n `668`, weak_sample_signal
- polymarket_volume_24h -> crypto_major_forward_1h_return_pct: corr `0.1218`, n `668`, weak_sample_signal
- polymarket_volume_24h -> crypto_alt_forward_1h_return_pct: corr `0.1051`, n `668`, weak_sample_signal
- news_risk_score -> crypto_alt_forward_1h_return_pct: corr `-0.0933`, n `668`, weak_sample_signal
- news_risk_score -> fx_forward_1h_return_pct: corr `0.0909`, n `668`, weak_sample_signal
- news_risk_score -> metal_forward_1h_return_pct: corr `0.0861`, n `668`, weak_sample_signal
- market_context_score -> metal_forward_1h_return_pct: corr `-0.0857`, n `668`, weak_sample_signal
- news_risk_score -> unknown_forward_1h_return_pct: corr `-0.0767`, n `668`, weak_sample_signal
