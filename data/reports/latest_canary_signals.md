# Latest Canary Signals

These are early-warning indicators for cross-market relationships. They are hypotheses to test, not trade signals by themselves.

- Updated: `2026-08-01T10:22:28.505407+00:00`
- Correlation status: `ready`
- Asset price records: `672`
- Minimum samples for correlation: `24`

## Current Signals

- baseline: score `0` - No elevated canary signal. Continue collecting samples.

## Class Returns

- 15m: commodity avg `-0.0222` n `12`; crypto_alt avg `0.1385` n `230`; crypto_major avg `0.0961` n `8`; equity avg `-0.0101` n `102`; fx avg `-0.0195` n `6`; index avg `0.0078` n `25`; metal avg `0.0047` n `20`; unknown avg `0.0264` n `781`
- 1h: commodity avg `0.0126` n `12`; crypto_alt avg `-0.1676` n `230`; crypto_major avg `-0.1375` n `8`; equity avg `-0.047` n `102`; fx avg `-0.0209` n `6`; index avg `0.0318` n `25`; metal avg `-0.0136` n `20`; unknown avg `0.0038` n `781`
- 4h: commodity avg `0.0372` n `12`; crypto_alt avg `-0.3356` n `230`; crypto_major avg `-0.2726` n `8`; equity avg `-0.1185` n `102`; fx avg `-0.0004` n `6`; index avg `0.0224` n `25`; metal avg `0.0019` n `20`; unknown avg `-0.0725` n `781`
- 24h: commodity avg `0.5376` n `12`; crypto_alt avg `0.168` n `230`; crypto_major avg `-1.2226` n `8`; equity avg `-2.7514` n `102`; fx avg `-0.0944` n `6`; index avg `-0.308` n `25`; metal avg `-0.056` n `20`; unknown avg `4.7633` n `764`

## Correlations

- flow_alert_score -> commodity_forward_1h_return_pct: corr `0.1072`, n `668`, weak_sample_signal
- polymarket_volume_24h -> crypto_alt_forward_1h_return_pct: corr `-0.1038`, n `668`, weak_sample_signal
- polymarket_volume_24h -> commodity_forward_1h_return_pct: corr `0.1035`, n `668`, weak_sample_signal
- risk_on_score -> crypto_major_forward_1h_return_pct: corr `-0.0862`, n `668`, weak_sample_signal
- flow_alert_score -> crypto_alt_forward_1h_return_pct: corr `-0.0761`, n `668`, weak_sample_signal
- news_risk_score -> commodity_forward_1h_return_pct: corr `-0.073`, n `668`, weak_sample_signal
- risk_on_score -> metal_forward_1h_return_pct: corr `0.0683`, n `668`, weak_sample_signal
- news_risk_score -> index_forward_1h_return_pct: corr `0.068`, n `668`, weak_sample_signal
- market_context_score -> metal_forward_1h_return_pct: corr `0.0648`, n `668`, weak_sample_signal
- market_context_score -> equity_forward_1h_return_pct: corr `-0.0645`, n `668`, weak_sample_signal
