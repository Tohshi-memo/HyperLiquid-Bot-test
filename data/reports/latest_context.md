# Latest Crypto Context

- Generated: `2026-07-11T19:55:59.530346+00:00`
- Market context score: `52.42`
- News risk score: `43.82`
- Macro risk score: `27.8`
- Risk-on score: `41.77`
- Articles: `57`
- Polymarket markets: `396`

- Sector reaction price records: `6500`
- Sector reaction patterns: `663`

- Asset universe count: `1158`
- Asset price history records: `672`

- Asset classes: `commodity:12, crypto_alt:230, crypto_major:8, equity:92, fx:6, index:25, metal:20, unknown:765`
- HIP-3 dexes: `abcd, cash, flx, hyna, km, mkts, para, vntl, xyz`

- Day/swing records: `None`
- Day/swing latest: `None`

## News Categories

- commodity: articles `2`, risk hits `3`, risk headline rate `1.0`, policy hits `1`
- crypto: articles `10`, risk hits `3`, risk headline rate `0.3`, policy hits `1`
- macro: articles `45`, risk hits `10`, risk headline rate `0.2`, policy hits `3`

## Headlines

- [macro] Morgan Stanley drops timely Honeywell stock opinion (Yahoo Finance)
- [macro] The SEC Is Rethinking Its Approach to ETFs. Here's What It Could Mean For Crypto Investors (Yahoo Finance)
- [macro] Polish PM pledges memorial to victims of WW2 'genocide by Ukrainian nationalists' (BBC News)
- [macro] Could You Retire Like Royalty in Vietnam With $400,000? (Yahoo Finance)
- [macro] Iran supreme leader calls for revenge after father's burial (BBC News)
- [macro] Which Tech ETF Wins for Your Portfolio, Fidelity's FTEC or Vanguard's VGT? (Yahoo Finance)
- [macro] Dow Jones Futures: Watch Nvidia, Micron, Sandisk, Robinhood As Market Sets Up; Big Earnings Due (Yahoo Finance)
- [crypto] Lending protocol Bonzo loses 77% of value locked as $9 million oracle exploit rattles Hedera (CoinDesk: Bitcoin, Ethereum, Crypto News and Price Data)
- [macro] US Democrat Ro Khanna says he was detained by armed Israeli settlers (BBC News)
- [macro] Bitcoin dropped 2% then rebounded after Trump said he's 'a big crypto guy' — here are experts takes on buying the dip (Yahoo Finance)
