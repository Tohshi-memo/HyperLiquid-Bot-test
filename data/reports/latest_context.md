# Latest Crypto Context

- Generated: `2026-08-13T06:37:28.478571+00:00`
- Market context score: `48.95`
- News risk score: `41.22`
- Macro risk score: `27.59`
- Risk-on score: `31.8`
- Articles: `45`
- Polymarket markets: `389`

- Sector reaction price records: `6500`
- Sector reaction patterns: `663`

- Asset universe count: `1201`
- Asset price history records: `672`

- Asset classes: `commodity:12, crypto_alt:230, crypto_major:8, equity:113, fx:6, index:25, metal:20, unknown:787`
- HIP-3 dexes: `abcd, cash, flx, hyna, km, mkts, para, vntl, xyz`

- Day/swing records: `8861`
- Day/swing latest: `2026-08-13T06:30:00+00:00`

## News Categories

- commodity: articles `4`, risk hits `6`, risk headline rate `1.0`, policy hits `0`
- crypto: articles `16`, risk hits `5`, risk headline rate `0.3125`, policy hits `3`
- macro: articles `25`, risk hits `3`, risk headline rate `0.12`, policy hits `0`

## Headlines

- [macro] UK economic growth slows between April and June (BBC News)
- [macro] Japan protests against Putin visit to disputed islands (BBC News)
- [commodity] Oil prices fluctuate as investors weigh falling demand against Middle East tensions (Energy)
- [crypto] Here’s what happened in crypto today (Cointelegraph.com News)
- [crypto] Metaplanet denies selling bitcoin worth $320 million (CoinDesk: Bitcoin, Ethereum, Crypto News and Price Data)
- [crypto] Bitcoin firms ask AI labs for same tools attackers already have (CoinDesk: Bitcoin, Ethereum, Crypto News and Price Data)
- [crypto] Crypto valuations could double as protocols link revenue to tokens: Bitwise CIO (Cointelegraph.com News)
- [macro] Putin can no longer claim victory in Ukraine, Nobel Peace Prize winner tells BBC (BBC News)
- [crypto] Metaplanet CEO shuts down Bitcoin sale speculation after $322M transfer (Cointelegraph.com News)
- [macro] BTS star V reveals hearing loss battle during livestream (BBC News)
