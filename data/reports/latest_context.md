# Latest Crypto Context

- Generated: `2026-08-08T13:52:27.291857+00:00`
- Market context score: `52.4`
- News risk score: `36.57`
- Macro risk score: `26.47`
- Risk-on score: `35.1`
- Articles: `30`
- Polymarket markets: `389`

- Sector reaction price records: `6500`
- Sector reaction patterns: `663`

- Asset universe count: `1197`
- Asset price history records: `672`

- Asset classes: `commodity:12, crypto_alt:230, crypto_major:8, equity:112, fx:6, index:25, metal:20, unknown:784`
- HIP-3 dexes: `abcd, cash, flx, hyna, km, mkts, para, vntl, xyz`

- Day/swing records: `8419`
- Day/swing latest: `2026-08-08T13:45:00+00:00`

## News Categories

- commodity: articles `1`, risk hits `0`, risk headline rate `0.0`, policy hits `0`
- crypto: articles `8`, risk hits `2`, risk headline rate `0.25`, policy hits `0`
- macro: articles `21`, risk hits `5`, risk headline rate `0.2381`, policy hits `1`

## Headlines

- [macro] US gifts $1bn to Colombia on new right-wing president's first day of office (BBC News)
- [macro] Is football AI-proof? Why tech investors wanted a slice of the World Cup (BBC News)
- [crypto] Bitcoin will never fall below $60K again: Nansen founder (Cointelegraph.com News)
- [macro] Could Copper’s Tariff-Driven Rally Be Running Out of Steam? (Yahoo Finance)
- [macro] Child among three killed in Russian missile attacks near Kyiv (BBC News)
- [macro] President Donald Trump Expected the Iran War to Drive the Stock Market "Down 20% to 25%" -- His Prophecy May Still Come True (Yahoo Finance)
- [macro] Nike (NKE) Is Betting Big on Basketball’s Next Generation. Will It Pay Off? (Yahoo Finance)
- [macro] Wall Street Sees a Multitrillion-Dollar Humanoid Robot Market. These 2 Industrial Stocks Sell the Parts. (Yahoo Finance)
- [macro] Trump's ex-lawyer Todd Blanche narrowly confirmed as US attorney general (BBC News)
- [macro] Reddit CEO Steve Huffman Sells 18,000 Shares for $2.9 Million (Yahoo Finance)
