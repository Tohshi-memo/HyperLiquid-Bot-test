# Latest Crypto Context

- Generated: `2026-08-28T09:37:23.465784+00:00`
- Market context score: `39.15`
- News risk score: `51.4`
- Macro risk score: `30.53`
- Risk-on score: `19.71`
- Articles: `36`
- Polymarket markets: `391`

- Sector reaction price records: `6500`
- Sector reaction patterns: `646`

- Asset universe count: `1222`
- Asset price history records: `672`

- Asset classes: `commodity:12, crypto_alt:231, crypto_major:8, equity:127, fx:6, index:26, metal:20, unknown:792`
- HIP-3 dexes: `abcd, cash, flx, hyna, io, km, mkts, para, vntl, xyz`

- Day/swing records: `10288`
- Day/swing latest: `2026-08-28T09:30:00+00:00`

## News Categories

- commodity: articles `2`, risk hits `2`, risk headline rate `1.0`, policy hits `0`
- crypto: articles `15`, risk hits `8`, risk headline rate `0.5333`, policy hits `1`
- macro: articles `19`, risk hits `5`, risk headline rate `0.2105`, policy hits `1`

## Headlines

- [crypto] ENA token rises 10% as Ethena puts revenue-funded token buybacks to vote (Cointelegraph.com News)
- [crypto] BitGo buys NYDIG trading arm to deepen institutional crypto push (Cointelegraph.com News)
- [crypto] Kraken users briefly locked out after a flood of sanctioned crypto transactions (CoinDesk: Bitcoin, Ethereum, Crypto News and Price Data)
- [crypto] Live updates: Bitcoin options worth $6.4 billion just expired as prices hover near $80,000 (CoinDesk: Bitcoin, Ethereum, Crypto News and Price Data)
- [crypto] IREN shares fall 8% as costly AI transition weighs on earnings (CoinDesk: Bitcoin, Ethereum, Crypto News and Price Data)
- [macro] A look back at the King who broke with tradition (BBC News)
- [macro] Nepal army rescues survivors from hydropower tunnel (BBC News)
- [macro] Trump signs order to rename Lake Ontario as Lake America (BBC News)
- [crypto] OneKey reproduces transaction replacement attack on outdated Ledger Ethereum app (Cointelegraph.com News)
- [macro] 'High risk' of lake breaching and causing second flood, warns China (BBC News)
