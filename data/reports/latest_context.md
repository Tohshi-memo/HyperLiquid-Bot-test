# Latest Crypto Context

- Generated: `2026-07-21T17:37:36.557739+00:00`
- Market context score: `53.26`
- News risk score: `42.19`
- Macro risk score: `23.43`
- Risk-on score: `40.62`
- Articles: `83`
- Polymarket markets: `396`

- Sector reaction price records: `6500`
- Sector reaction patterns: `663`

- Asset universe count: `1170`
- Asset price history records: `672`

- Asset classes: `commodity:12, crypto_alt:230, crypto_major:8, equity:98, fx:6, index:25, metal:20, unknown:771`
- HIP-3 dexes: `abcd, cash, flx, hyna, km, mkts, para, vntl, xyz`

- Day/swing records: `6758`
- Day/swing latest: `2026-07-21T17:30:00+00:00`

## News Categories

- commodity: articles `3`, risk hits `1`, risk headline rate `0.3333`, policy hits `1`
- crypto: articles `26`, risk hits `8`, risk headline rate `0.2308`, policy hits `1`
- macro: articles `54`, risk hits `9`, risk headline rate `0.1667`, policy hits `1`

## Headlines

- [macro] Carney looking at 'all options' as Trump announces 50% tariffs on Canada (BBC News)
- [macro] Carney looking at 'all options' as Trump announces 50% tariffs on Canada (BBC News)
- [macro] Palantir Stock Is Down More Than 35% from Its Peak. Is It Finally a Buy? (Yahoo Finance)
- [macro] XXI stock plunges 18% after CEO's abrupt exit (Yahoo Finance)
- [macro] Oracle stock makes rattling move after major setback (Yahoo Finance)
- [macro] Why Equifax Stock Is Tumbling Today (Yahoo Finance)
- [macro] Monolithic Power Systems (MPWR) vs. TXN and ADI: Are Investors Paying Too Much for AI Power Growth? (Yahoo Finance)
- [macro] Union plans rail strikes over train safety concerns (BBC News)
- [macro] B&M fined £2m over serious food hygiene failings at Birmingham shops (BBC News)
- [macro] Two French firefighters die tackling blaze near Bordeaux airport (BBC News)
