# Latest Crypto Context

- Generated: `2026-06-15T07:07:31.262163+00:00`
- Market context score: `41.03`
- News risk score: `45.73`
- Macro risk score: `26.39`
- Risk-on score: `17.53`
- Articles: `33`
- Polymarket markets: `402`

- Sector reaction price records: `6500`
- Sector reaction patterns: `629`

- Asset universe count: `1041`
- Asset price history records: `672`

- Asset classes: `commodity:12, crypto_alt:228, crypto_major:8, equity:74, fx:6, index:23, metal:16, unknown:674`
- HIP-3 dexes: `abcd, flx, hyna, km, para, vntl, xyz`

- Day/swing records: `None`
- Day/swing latest: `None`

## News Categories

- commodity: articles `1`, risk hits `0`, risk headline rate `0.0`, policy hits `0`
- crypto: articles `11`, risk hits `6`, risk headline rate `0.4545`, policy hits `1`
- macro: articles `21`, risk hits `5`, risk headline rate `0.2381`, policy hits `0`

## Headlines

- [macro] Norwegian crown princess's son found guilty of two counts of rape (BBC News)
- [macro] Trump and thousands of others watch UFC fight on White House lawn (BBC News)
- [commodity] U.S. crude drops nearly 5% as Washington-Tehran deal is set to open Hormuz Strait (Energy)
- [crypto] Bitcoin nears $66K as Trump says US has peace deal with Iran (Cointelegraph.com News)
- [macro] US musician Oliver Tree dies in helicopter collision in Brazil (BBC News)
- [crypto] Bitcoin traders have a reason to watch Tuesday's BOJ rate decision. Yen shorts are at a nine-year high (CoinDesk: Bitcoin, Ethereum, Crypto News and Price Data)
- [macro] Oil prices slide after US-Iran deal announced (BBC News)
- [macro] Oil prices slide after US-Iran deal announced (BBC News)
- [crypto] CFTC sues New Mexico over prediction market jurisdiction (Cointelegraph.com News)
- [crypto] Aztec Connect’s abandoned smart contract exploited for $2.1M (Cointelegraph.com News)
