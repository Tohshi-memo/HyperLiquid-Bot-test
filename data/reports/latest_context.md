# Latest Crypto Context

- Generated: `2026-09-07T00:06:01.002180+00:00`
- Market context score: `56.18`
- News risk score: `29.77`
- Macro risk score: `30.33`
- Risk-on score: `39.22`
- Articles: `50`
- Polymarket markets: `391`

- Sector reaction price records: `6500`
- Sector reaction patterns: `629`

- Asset universe count: `1234`
- Asset price history records: `672`

- Asset classes: `commodity:12, crypto_alt:232, crypto_major:8, equity:134, fx:6, index:26, metal:20, unknown:796`
- HIP-3 dexes: `abcd, cash, flx, hyna, io, km, mkts, para, vntl, xyz`

- Day/swing records: `11196`
- Day/swing latest: `2026-09-07T00:00:00+00:00`

## News Categories

- commodity: articles `1`, risk hits `1`, risk headline rate `1.0`, policy hits `0`
- crypto: articles `5`, risk hits `0`, risk headline rate `0.0`, policy hits `0`
- macro: articles `44`, risk hits `14`, risk headline rate `0.2727`, policy hits `4`

## Headlines

- [macro] Austria's Islamic headscarf ban in force as under-14s go back to school (BBC News)
- [macro] Dow Jones Futures Fall With Iran, Apple, Inflation In Focus; Nvidia, Micron, Sandisk Flash Buy Signals (Yahoo Finance)
- [macro] Chancellor to unveil growth plan with £150m fund for northern firms (BBC News)
- [macro] Gap closed 350 stores and now has an Old Navy problem (Yahoo Finance)
- [macro] American Airlines (AAL) Leans on its Smallest Long-Haul Jet to Chase Higher-Margin Routes (Yahoo Finance)
- [macro] What a $10,000 Investment in the Vanguard S&P 500 ETF (VOO) a Decade Ago Is Worth Today (Yahoo Finance)
- [macro] Blackstone (BX) Cashes Out Up to $1.3 Billion from its India Real Estate Bet (Yahoo Finance)
- [macro] Warren Buffett has a stark message for stock market investors (Yahoo Finance)
- [macro] The £70 refund letter that isn't a scam (BBC News)
- [macro] The volunteers risking their lives to defuse Indonesia's wildfire 'carbon bombs' (BBC News)
