# Latest HIP-4 Outcome Markets

- Generated: `2026-06-16T16:07:45.104812+00:00`
- Info endpoint: `https://api.hyperliquid.xyz/info`
- Outcome markets: `122`
- Outcome sides (rows): `244`

## Markets by Underlying

- BTC: `1`
- ETH: `2`
- HYPE: `1`
- SOL: `1`
- unknown: `117`

## Markets by Class

- fallback: `1`
- namedOutcome: `3`
- priceBinary: `4`
- unknown: `114`

## Current Implied Probabilities

- Haiti [No] symbol `#1921` underlying `n/a` prob `0.9999` price_source `allMids` vol24h `0.0` oi `0.0`
- Curacao [No] symbol `#1841` underlying `n/a` prob `0.9993` price_source `allMids` vol24h `0.0` oi `0.0`
- South Africa [No] symbol `#2101` underlying `n/a` prob `0.9993` price_source `allMids` vol24h `0.0` oi `0.0`
- Panama [No] symbol `#2031` underlying `n/a` prob `0.9992` price_source `allMids` vol24h `0.0` oi `0.0`
- Iraq [No] symbol `#1941` underlying `n/a` prob `0.9992` price_source `allMids` vol24h `0.0` oi `0.0`
- Uzbekistan [No] symbol `#2191` underlying `n/a` prob `0.9992` price_source `allMids` vol24h `0.0` oi `0.0`
- Saudi Arabia [No] symbol `#2071` underlying `n/a` prob `0.9992` price_source `allMids` vol24h `0.0` oi `0.0`
- New Zealand [No] symbol `#2011` underlying `n/a` prob `0.9992` price_source `allMids` vol24h `0.0` oi `0.0`
- Congo DR [No] symbol `#1821` underlying `n/a` prob `0.9991` price_source `allMids` vol24h `0.0` oi `0.0`
- Czechia [No] symbol `#1851` underlying `n/a` prob `0.999` price_source `allMids` vol24h `0.0` oi `0.0`
- Qatar [No] symbol `#2061` underlying `n/a` prob `0.9989` price_source `allMids` vol24h `0.0` oi `0.0`
- Iran [No] symbol `#1931` underlying `n/a` prob `0.9989` price_source `allMids` vol24h `0.0` oi `0.0`
- Canada [No] symbol `#1791` underlying `n/a` prob `0.9988` price_source `allMids` vol24h `0.0` oi `0.0`
- Bosnia and Herzegovina [No] symbol `#1771` underlying `n/a` prob `0.9985` price_source `allMids` vol24h `0.0` oi `0.0`
- Egypt [No] symbol `#1871` underlying `n/a` prob `0.9984` price_source `allMids` vol24h `0.0` oi `0.0`
- Algeria [No] symbol `#1721` underlying `n/a` prob `0.9984` price_source `allMids` vol24h `0.0` oi `0.0`
- Scotland [No] symbol `#2081` underlying `n/a` prob `0.9979` price_source `allMids` vol24h `0.0` oi `0.0`
- Paraguay [No] symbol `#2041` underlying `n/a` prob `0.9974` price_source `allMids` vol24h `0.0` oi `0.0`
- Cape Verde [No] symbol `#1801` underlying `n/a` prob `0.9974` price_source `allMids` vol24h `0.0` oi `0.0`
- Tunisia [No] symbol `#2151` underlying `n/a` prob `0.9974` price_source `allMids` vol24h `0.0` oi `0.0`
- Ghana [No] symbol `#1911` underlying `n/a` prob `0.9973` price_source `allMids` vol24h `0.0` oi `0.0`
- Austria [No] symbol `#1751` underlying `n/a` prob `0.9966` price_source `allMids` vol24h `0.0` oi `0.0`
- South Korea [No] symbol `#2111` underlying `n/a` prob `0.9951` price_source `allMids` vol24h `0.0` oi `0.0`
- Jordan [No] symbol `#1971` underlying `n/a` prob `0.9949` price_source `allMids` vol24h `0.0` oi `0.0`
- Australia [No] symbol `#1741` underlying `n/a` prob `0.9949` price_source `allMids` vol24h `0.0` oi `0.0`

## Top by 24h Volume

- June Fed rate change [Change] symbol `#1040` underlying `n/a` prob `0.0084` price_source `allMids` vol24h `0.0` oi `0.0`
- June Fed rate change [No Change] symbol `#1041` underlying `n/a` prob `0.9916` price_source `allMids` vol24h `0.0` oi `0.0`
- Fallback [Yes] symbol `#1710` underlying `n/a` prob `0.5` price_source `allMids` vol24h `0.0` oi `0.0`
- Fallback [No] symbol `#1711` underlying `n/a` prob `0.5` price_source `allMids` vol24h `0.0` oi `0.0`
- Algeria [Yes] symbol `#1720` underlying `n/a` prob `0.0016` price_source `allMids` vol24h `0.0` oi `0.0`
- Algeria [No] symbol `#1721` underlying `n/a` prob `0.9984` price_source `allMids` vol24h `0.0` oi `0.0`
- Argentina [Yes] symbol `#1730` underlying `n/a` prob `0.0966` price_source `allMids` vol24h `0.0` oi `0.0`
- Argentina [No] symbol `#1731` underlying `n/a` prob `0.9034` price_source `allMids` vol24h `0.0` oi `0.0`
- Australia [Yes] symbol `#1740` underlying `n/a` prob `0.0052` price_source `allMids` vol24h `0.0` oi `0.0`
- Australia [No] symbol `#1741` underlying `n/a` prob `0.9949` price_source `allMids` vol24h `0.0` oi `0.0`
- Austria [Yes] symbol `#1750` underlying `n/a` prob `0.0034` price_source `allMids` vol24h `0.0` oi `0.0`
- Austria [No] symbol `#1751` underlying `n/a` prob `0.9966` price_source `allMids` vol24h `0.0` oi `0.0`
- Belgium [Yes] symbol `#1760` underlying `n/a` prob `0.0227` price_source `allMids` vol24h `0.0` oi `0.0`
- Belgium [No] symbol `#1761` underlying `n/a` prob `0.9773` price_source `allMids` vol24h `0.0` oi `0.0`
- Bosnia and Herzegovina [Yes] symbol `#1770` underlying `n/a` prob `0.0015` price_source `allMids` vol24h `0.0` oi `0.0`
- Bosnia and Herzegovina [No] symbol `#1771` underlying `n/a` prob `0.9985` price_source `allMids` vol24h `0.0` oi `0.0`
- Brazil [Yes] symbol `#1780` underlying `n/a` prob `0.0591` price_source `allMids` vol24h `0.0` oi `0.0`
- Brazil [No] symbol `#1781` underlying `n/a` prob `0.9409` price_source `allMids` vol24h `0.0` oi `0.0`
- Canada [Yes] symbol `#1790` underlying `n/a` prob `0.0012` price_source `allMids` vol24h `0.0` oi `0.0`
- Canada [No] symbol `#1791` underlying `n/a` prob `0.9988` price_source `allMids` vol24h `0.0` oi `0.0`
- Cape Verde [Yes] symbol `#1800` underlying `n/a` prob `0.0026` price_source `allMids` vol24h `0.0` oi `0.0`
- Cape Verde [No] symbol `#1801` underlying `n/a` prob `0.9974` price_source `allMids` vol24h `0.0` oi `0.0`
- Colombia [Yes] symbol `#1810` underlying `n/a` prob `0.0168` price_source `allMids` vol24h `0.0` oi `0.0`
- Colombia [No] symbol `#1811` underlying `n/a` prob `0.9832` price_source `allMids` vol24h `0.0` oi `0.0`
- Congo DR [Yes] symbol `#1820` underlying `n/a` prob `0.0009` price_source `allMids` vol24h `0.0` oi `0.0`

## Top by Open Interest

- June Fed rate change [Change] symbol `#1040` underlying `n/a` prob `0.0084` price_source `allMids` vol24h `0.0` oi `0.0`
- June Fed rate change [No Change] symbol `#1041` underlying `n/a` prob `0.9916` price_source `allMids` vol24h `0.0` oi `0.0`
- Fallback [Yes] symbol `#1710` underlying `n/a` prob `0.5` price_source `allMids` vol24h `0.0` oi `0.0`
- Fallback [No] symbol `#1711` underlying `n/a` prob `0.5` price_source `allMids` vol24h `0.0` oi `0.0`
- Algeria [Yes] symbol `#1720` underlying `n/a` prob `0.0016` price_source `allMids` vol24h `0.0` oi `0.0`
- Algeria [No] symbol `#1721` underlying `n/a` prob `0.9984` price_source `allMids` vol24h `0.0` oi `0.0`
- Argentina [Yes] symbol `#1730` underlying `n/a` prob `0.0966` price_source `allMids` vol24h `0.0` oi `0.0`
- Argentina [No] symbol `#1731` underlying `n/a` prob `0.9034` price_source `allMids` vol24h `0.0` oi `0.0`
- Australia [Yes] symbol `#1740` underlying `n/a` prob `0.0052` price_source `allMids` vol24h `0.0` oi `0.0`
- Australia [No] symbol `#1741` underlying `n/a` prob `0.9949` price_source `allMids` vol24h `0.0` oi `0.0`
- Austria [Yes] symbol `#1750` underlying `n/a` prob `0.0034` price_source `allMids` vol24h `0.0` oi `0.0`
- Austria [No] symbol `#1751` underlying `n/a` prob `0.9966` price_source `allMids` vol24h `0.0` oi `0.0`
- Belgium [Yes] symbol `#1760` underlying `n/a` prob `0.0227` price_source `allMids` vol24h `0.0` oi `0.0`
- Belgium [No] symbol `#1761` underlying `n/a` prob `0.9773` price_source `allMids` vol24h `0.0` oi `0.0`
- Bosnia and Herzegovina [Yes] symbol `#1770` underlying `n/a` prob `0.0015` price_source `allMids` vol24h `0.0` oi `0.0`
- Bosnia and Herzegovina [No] symbol `#1771` underlying `n/a` prob `0.9985` price_source `allMids` vol24h `0.0` oi `0.0`
- Brazil [Yes] symbol `#1780` underlying `n/a` prob `0.0591` price_source `allMids` vol24h `0.0` oi `0.0`
- Brazil [No] symbol `#1781` underlying `n/a` prob `0.9409` price_source `allMids` vol24h `0.0` oi `0.0`
- Canada [Yes] symbol `#1790` underlying `n/a` prob `0.0012` price_source `allMids` vol24h `0.0` oi `0.0`
- Canada [No] symbol `#1791` underlying `n/a` prob `0.9988` price_source `allMids` vol24h `0.0` oi `0.0`
- Cape Verde [Yes] symbol `#1800` underlying `n/a` prob `0.0026` price_source `allMids` vol24h `0.0` oi `0.0`
- Cape Verde [No] symbol `#1801` underlying `n/a` prob `0.9974` price_source `allMids` vol24h `0.0` oi `0.0`
- Colombia [Yes] symbol `#1810` underlying `n/a` prob `0.0168` price_source `allMids` vol24h `0.0` oi `0.0`
- Colombia [No] symbol `#1811` underlying `n/a` prob `0.9832` price_source `allMids` vol24h `0.0` oi `0.0`
- Congo DR [Yes] symbol `#1820` underlying `n/a` prob `0.0009` price_source `allMids` vol24h `0.0` oi `0.0`

## Request Errors

- None.

## Request Warnings

- `outcomeState: 422 Client Error: Unprocessable Entity for url: https://api.hyperliquid.xyz/info`

Public output stores aggregate HIP-4 outcome data only. It is not a trade signal.
