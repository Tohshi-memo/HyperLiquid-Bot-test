# Latest Crypto Context

- Generated: `2026-08-08T14:52:27.930081+00:00`
- Market context score: `53.57`
- News risk score: `36.64`
- Macro risk score: `26.66`
- Risk-on score: `37.83`
- Articles: `38`
- Polymarket markets: `389`

- Sector reaction price records: `6500`
- Sector reaction patterns: `663`

- Asset universe count: `1197`
- Asset price history records: `672`

- Asset classes: `commodity:12, crypto_alt:230, crypto_major:8, equity:112, fx:6, index:25, metal:20, unknown:784`
- HIP-3 dexes: `abcd, cash, flx, hyna, km, mkts, para, vntl, xyz`

- Day/swing records: `8423`
- Day/swing latest: `2026-08-08T14:45:00+00:00`

## News Categories

- commodity: articles `1`, risk hits `0`, risk headline rate `0.0`, policy hits `0`
- crypto: articles `8`, risk hits `2`, risk headline rate `0.25`, policy hits `0`
- macro: articles `29`, risk hits `7`, risk headline rate `0.2414`, policy hits `1`

## Headlines

- [macro] Jeff Bezos' Amazon Just Raised Its AI Spending to $220 Billion for 2026. Here's What That Capex Hike Means for Investors. (Yahoo Finance)
- [macro] eBay Slid This Past Month: Why a Leading Analyst Expects 32% Growth Ahead Anyway (Yahoo Finance)
- [macro] Spain imposes border controls against Italy as row over Ceuta migrant influx intensifies (BBC News)
- [macro] Stock Market Week Ahead: Bullish Signals Amid 'Dirty' Volume (Yahoo Finance)
- [macro] Everyone's Chasing AI Data Center Stocks. This 5%-Yielding Dividend Stock Already Owns a Piece of the Boom. (Yahoo Finance)
- [macro] Executive Parts Ways With 3,820 Shares of Lamb Weston Stock (Yahoo Finance)
- [crypto] Trillions in institutional money to flow into bitcoin, says Bitwise's Matt Hougan (CoinDesk: Bitcoin, Ethereum, Crypto News and Price Data)
- [macro] Jim Cramer Favors Fortinet (FTNT) Alongside CrowdStrike (CRWD) and Palo Alto (PANW) (Yahoo Finance)
- [macro] US gifts $1bn to Colombia on new right-wing president's first day of office (BBC News)
- [macro] Is football AI-proof? Why tech investors wanted a slice of the World Cup (BBC News)
