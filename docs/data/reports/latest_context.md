# Latest Crypto Context

- Generated: `2026-06-26T12:52:30.740072+00:00`
- Market context score: `57.61`
- News risk score: `34.79`
- Macro risk score: `26.48`
- Risk-on score: `45.19`
- Articles: `41`
- Polymarket markets: `393`

- Sector reaction price records: `6500`
- Sector reaction patterns: `663`

- Asset universe count: `1148`
- Asset price history records: `672`

- Asset classes: `commodity:12, crypto_alt:228, crypto_major:8, equity:86, fx:6, index:23, metal:20, unknown:765`
- HIP-3 dexes: `abcd, cash, flx, hyna, km, mkts, para, vntl, xyz`

- Day/swing records: `4389`
- Day/swing latest: `2026-06-26T12:45:00+00:00`

## News Categories

- commodity: articles `2`, risk hits `2`, risk headline rate `0.5`, policy hits `0`
- crypto: articles `20`, risk hits `4`, risk headline rate `0.2`, policy hits `4`
- macro: articles `19`, risk hits `3`, risk headline rate `0.1579`, policy hits `1`

## Headlines

- [crypto] Wall Street's IPO revival hasn't reached dot-com euphoria levels, Goldman Sachs says (CoinDesk: Bitcoin, Ethereum, Crypto News and Price Data)
- [crypto] Australian regulator extends no-action period for crypto licensing (Cointelegraph.com News)
- [crypto] AscendEX withdrawal complaints mount as ZachXBT questions reserves (Cointelegraph.com News)
- [crypto] With crypto ending the first half in the red, bitcoin's solace is it beat Strategy (CoinDesk: Bitcoin, Ethereum, Crypto News and Price Data)
- [macro] UN pauses Strait of Hormuz evacuation plan after cargo ship attacked (BBC News)
- [macro] Daily – Vickers Top Insider Picks for 06/26/2026 (Yahoo Finance)
- [macro] Daily – Vickers Top Buyers & Sellers for 06/26/2026 (Yahoo Finance)
- [commodity] U.S. crude oil falls below $70, resuming losses after attack on cargo ship near Oman (Energy)
- [crypto] Bitcoin bounces from $58,000 as derivatives signal more pain in the pipeline (CoinDesk: Bitcoin, Ethereum, Crypto News and Price Data)
- [macro] Market Digest: CCL, SPCX (Yahoo Finance)
