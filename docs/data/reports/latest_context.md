# Latest Crypto Context

- Generated: `2026-05-27T23:22:17.299158+00:00`
- Market context score: `47.65`
- News risk score: `45.15`
- Macro risk score: `25.59`
- Risk-on score: `31.45`
- Articles: `69`
- Polymarket markets: `396`

- Sector reaction price records: `6500`
- Sector reaction patterns: `629`

- Asset universe count: `781`
- Asset price history records: `672`

- Asset classes: `commodity:12, crypto_alt:228, crypto_major:8, equity:67, fx:6, index:23, metal:18, unknown:419`
- HIP-3 dexes: `abcd, cash, flx, hyna, km, para, vntl, xyz`

- Day/swing records: `1862`
- Day/swing latest: `2026-05-27T23:15:00+00:00`

## News Categories

- commodity: articles `3`, risk hits `1`, risk headline rate `0.3333`, policy hits `0`
- crypto: articles `45`, risk hits `13`, risk headline rate `0.2889`, policy hits `3`
- macro: articles `20`, risk hits `5`, risk headline rate `0.2`, policy hits `0`
- policy: articles `1`, risk hits `0`, risk headline rate `0.0`, policy hits `0`

## Headlines

- [crypto] Crypto card monthly transaction volume surges 230% from 2025 (Cointelegraph.com News)
- [crypto] Bitcoin miner inflows to Binance soar as BTC struggles to hold uptrend: Is $70K next? (Cointelegraph.com News)
- [macro] The world's carmakers are struggling to compete with China (BBC News)
- [crypto] Google engineer insider-traded search results on Polymarket, Feds allege (CoinDesk: Bitcoin, Ethereum, Crypto News and Price Data)
- [crypto] Polymarket weighs KYC requirements amid global crackdown on prediction markets (Cointelegraph.com News)
- [macro] One in six young people will not be in work or training in five years without action, report warns (BBC News)
- [crypto] The crypto industry’s massive political war chest is starting to lean Republican ahead of midterms (CoinDesk: Bitcoin, Ethereum, Crypto News and Price Data)
- [crypto] Orca, Streamex roll out secondary trading infrastructure for tokenized securities (Cointelegraph.com News)
- [macro] Israel issues evacuation order for swathes of southern Lebanon (BBC News)
- [crypto] PACs laud Texas primary wins, look to back more pro-crypto candidates (Cointelegraph.com News)
