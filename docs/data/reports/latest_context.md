# Latest Crypto Context

- Generated: `2026-05-22T19:52:16.595715+00:00`
- Market context score: `44.16`
- News risk score: `48.58`
- Macro risk score: `29.0`
- Risk-on score: `27.89`
- Articles: `66`
- Polymarket markets: `399`

- Sector reaction price records: `6500`
- Sector reaction patterns: `629`

- Asset universe count: `748`
- Asset price history records: `672`

- Asset classes: `commodity:12, crypto_alt:228, crypto_major:8, equity:67, fx:6, index:23, metal:18, unknown:386`
- HIP-3 dexes: `abcd, cash, flx, hyna, km, para, vntl, xyz`

- Day/swing records: `1388`
- Day/swing latest: `2026-05-22T19:45:00+00:00`

## News Categories

- commodity: articles `4`, risk hits `4`, risk headline rate `0.75`, policy hits `1`
- crypto: articles `35`, risk hits `14`, risk headline rate `0.3429`, policy hits `2`
- macro: articles `26`, risk hits `6`, risk headline rate `0.1923`, policy hits `1`
- policy: articles `1`, risk hits `0`, risk headline rate `0.0`, policy hits `0`

## Headlines

- [macro] Tulsi Gabbard to resign as US director of national intelligence (BBC News)
- [crypto] F2Pool founder who controls 11% of bitcoin's hashrate to lead first SpaceX mission to Mars (CoinDesk: Bitcoin, Ethereum, Crypto News and Price Data)
- [crypto] Robinhood Crypto COO Tanya Denisova is leaving company amid revenue slowdown (CoinDesk: Bitcoin, Ethereum, Crypto News and Price Data)
- [macro] Russia's Putin vows retaliation after accusing Ukraine of hitting student dormitory (BBC News)
- [commodity] Oil prices post weekly loss as U.S. and Iran signal progress toward a deal (Energy)
- [macro] Bank boss sorry after describing workers as 'lower value human capital' (BBC News)
- [macro] Trump is putting pressure on Cuba - why and to what end? (BBC News)
- [crypto] SEC Commissioner Peirce counters views that crypto rule will foster synthetic tokens (CoinDesk: Bitcoin, Ethereum, Crypto News and Price Data)
- [macro] Trump wants new Fed chair to be 'totally independent' (BBC News)
- [macro] Bodies of Italian divers did not have optimal equipment, says rescuer (BBC News)
