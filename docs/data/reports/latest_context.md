# Latest Crypto Context

- Generated: `2026-05-03T21:00:29.179925+00:00`
- Market context score: `59.26`
- News risk score: `40.33`
- Macro risk score: `23.16`
- Risk-on score: `52.28`
- Articles: `53`
- Polymarket markets: `2`

- Asset universe count: `613`
- Asset price history records: `203`

- Asset classes: `commodity:7, crypto_alt:223, crypto_major:7, equity:42, fx:4, index:9, metal:7, unknown:314`
- HIP-3 dexes: `xyz`

- Day/swing records: `216`
- Day/swing latest: `2026-05-03T21:00:00+00:00`

## News Categories

- commodity: articles `2`, risk hits `2`, risk headline rate `0.5`, policy hits `0`
- crypto: articles `10`, risk hits `3`, risk headline rate `0.3`, policy hits `0`
- macro: articles `41`, risk hits `7`, risk headline rate `0.1707`, policy hits `0`

## Headlines

- [macro] Bank of America makes fresh call on Boeing stock price (Yahoo Finance)
- [macro] Three dead in suspected virus outbreak on Atlantic cruise ship (BBC News)
- [macro] Are People Who Live Within Their Means 'Suckers,' While Others Are Living Large With Massive Debt And Do Not Seem To Care? (Yahoo Finance)
- [macro] Prediction: This Will Be the Top-Performing Artificial Intelligence (AI) Semiconductor Stock Over the Next Year. (Hint: It's Not Nvidia, Broadcom, or Micron.) (Yahoo Finance)
- [macro] Iran says US has responded to its latest peace proposal (BBC News)
- [macro] Value Legend Bill Miller Just Bought Shares in These 2 Stocks (Yahoo Finance)
- [macro] NRG Energy Annual Meeting: Shareholder Special-Meeting Proposal Passes as CEO Larry Coben Steps Down (Yahoo Finance)
- [macro] Is Verisk Analytics, Inc. (VRSK) A Good Stock To Buy Now? (Yahoo Finance)
- [macro] Is Vestis Corporation (VSTS) A Good Stock To Buy Now? (Yahoo Finance)
- [macro] Is Walker & Dunlop, Inc. (WD) A Good Stock To Buy Now? (Yahoo Finance)
