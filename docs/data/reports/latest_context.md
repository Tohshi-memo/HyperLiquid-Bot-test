# Latest Crypto Context

- Generated: `2026-07-14T19:22:27.285282+00:00`
- Market context score: `44.72`
- News risk score: `44.26`
- Macro risk score: `26.93`
- Risk-on score: `24.71`
- Articles: `68`
- Polymarket markets: `397`

- Sector reaction price records: `6500`
- Sector reaction patterns: `663`

- Asset universe count: `1161`
- Asset price history records: `672`

- Asset classes: `commodity:12, crypto_alt:230, crypto_major:8, equity:92, fx:6, index:25, metal:20, unknown:768`
- HIP-3 dexes: `abcd, cash, flx, hyna, km, mkts, para, vntl, xyz`

- Day/swing records: `6108`
- Day/swing latest: `2026-07-14T19:15:00+00:00`

## News Categories

- commodity: articles `5`, risk hits `3`, risk headline rate `0.4`, policy hits `1`
- crypto: articles `35`, risk hits `13`, risk headline rate `0.2571`, policy hits `5`
- macro: articles `25`, risk hits `7`, risk headline rate `0.2`, policy hits `1`
- policy: articles `3`, risk hits `0`, risk headline rate `0.0`, policy hits `1`

## Headlines

- [commodity] Oil prices off highs after Trump abandons 20% protection fee on Strait of Hormuz traffic (Energy)
- [macro] Strait of Hormuz remains a fault line as Iran and US drift back into war (BBC News)
- [macro] Colombian national killed by ICE agent during operation in Maine (BBC News)
- [crypto] Some U.S. Senate Democrats come out against Clarity Act, calling it a 'corrupt' bill (CoinDesk: Bitcoin, Ethereum, Crypto News and Price Data)
- [policy] Bowman, Responsible Innovation and Financial Inclusion (FRB: Speeches)
- [commodity] Apple in talks with startup that shrinks AI models to run on an iPhone (Energy)
- [macro] Billionaire Warren Buffett stops donations to Bill Gates charity (BBC News)
- [policy] Minutes of the Board's discount rate meetings on June 8 and June 17, 2026 (FRB: Press Release - All Releases)
- [macro] Analyst Report: Fedex Corp (Yahoo Finance)
- [macro] Analyst Report: JPMorgan Chase & Co (Yahoo Finance)
