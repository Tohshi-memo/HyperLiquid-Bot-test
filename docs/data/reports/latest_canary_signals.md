# Latest Canary Signals

These are early-warning indicators for cross-market relationships. They are hypotheses to test, not trade signals by themselves.

- Updated: `2026-07-18T17:26:33.708247+00:00`
- Correlation status: `ready`
- Asset price records: `672`
- Minimum samples for correlation: `24`

## Current Signals

- baseline: score `0` - No elevated canary signal. Continue collecting samples.

## Class Returns

- 15m: commodity avg `0.0923` n `12`; crypto_alt avg `-0.011` n `230`; crypto_major avg `-0.033` n `8`; equity avg `0.0324` n `96`; fx avg `0.0` n `6`; index avg `0.0064` n `25`; metal avg `-0.0015` n `20`; unknown avg `-0.0544` n `770`
- 1h: commodity avg `0.0777` n `12`; crypto_alt avg `0.1247` n `230`; crypto_major avg `0.1045` n `8`; equity avg `0.0502` n `96`; fx avg `-0.0111` n `6`; index avg `-0.0004` n `25`; metal avg `-0.0075` n `20`; unknown avg `0.011` n `770`
- 4h: commodity avg `0.1051` n `12`; crypto_alt avg `0.3547` n `230`; crypto_major avg `0.4911` n `8`; equity avg `0.011` n `96`; fx avg `-0.0582` n `6`; index avg `-0.0181` n `25`; metal avg `-0.0417` n `20`; unknown avg `0.0123` n `770`
- 24h: commodity avg `0.3278` n `12`; crypto_alt avg `-1.0571` n `230`; crypto_major avg `-0.2087` n `8`; equity avg `-1.5802` n `96`; fx avg `-0.1126` n `6`; index avg `-0.1401` n `25`; metal avg `-0.0429` n `20`; unknown avg `-0.1494` n `737`

## Correlations

- flow_alert_score -> commodity_forward_1h_return_pct: corr `0.1352`, n `668`, weak_sample_signal
- news_risk_score -> metal_forward_1h_return_pct: corr `0.1127`, n `668`, weak_sample_signal
- flow_alert_score -> crypto_alt_forward_1h_return_pct: corr `-0.0994`, n `668`, weak_sample_signal
- market_context_score -> index_forward_1h_return_pct: corr `-0.0968`, n `668`, weak_sample_signal
- news_risk_score -> index_forward_1h_return_pct: corr `0.0913`, n `668`, weak_sample_signal
- flow_alert_score -> crypto_major_forward_1h_return_pct: corr `-0.0886`, n `668`, weak_sample_signal
- market_context_score -> metal_forward_1h_return_pct: corr `-0.0886`, n `668`, weak_sample_signal
- market_context_score -> equity_forward_1h_return_pct: corr `-0.0841`, n `668`, weak_sample_signal
- flow_alert_score -> fx_forward_1h_return_pct: corr `0.0818`, n `668`, weak_sample_signal
- risk_on_score -> equity_forward_1h_return_pct: corr `-0.0798`, n `668`, weak_sample_signal
