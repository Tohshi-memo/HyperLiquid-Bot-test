# Latest Canary Signals

These are early-warning indicators for cross-market relationships. They are hypotheses to test, not trade signals by themselves.

- Updated: `2026-09-08T16:22:28.400123+00:00`
- Correlation status: `ready`
- Asset price records: `672`
- Minimum samples for correlation: `24`

## Current Signals

- baseline: score `0` - No elevated canary signal. Continue collecting samples.

## Class Returns

- 15m: commodity avg `0.0147` n `12`; crypto_alt avg `-0.301` n `233`; crypto_major avg `-0.2693` n `8`; equity avg `-0.1988` n `134`; fx avg `0.0111` n `6`; index avg `-0.0209` n `26`; metal avg `-0.0003` n `20`; unknown avg `0.5716` n `797`
- 1h: commodity avg `-0.1708` n `12`; crypto_alt avg `0.0048` n `232`; crypto_major avg `0.3036` n `8`; equity avg `-0.0015` n `134`; fx avg `-0.0102` n `6`; index avg `-0.0203` n `26`; metal avg `-0.0777` n `20`; unknown avg `0.7716` n `795`
- 4h: commodity avg `-0.591` n `12`; crypto_alt avg `0.8462` n `232`; crypto_major avg `0.8977` n `8`; equity avg `0.9874` n `134`; fx avg `0.0258` n `6`; index avg `0.0497` n `26`; metal avg `0.0308` n `20`; unknown avg `0.0027` n `775`
- 24h: commodity avg `-0.2956` n `12`; crypto_alt avg `1.1446` n `232`; crypto_major avg `0.5673` n `8`; equity avg `1.03` n `134`; fx avg `-0.0598` n `6`; index avg `-0.0363` n `26`; metal avg `-0.0998` n `20`; unknown avg `7061.5904` n `708`

## Correlations

- news_risk_score -> metal_forward_1h_return_pct: corr `0.1262`, n `668`, weak_sample_signal
- news_risk_score -> equity_forward_1h_return_pct: corr `0.102`, n `668`, weak_sample_signal
- news_risk_score -> unknown_forward_1h_return_pct: corr `0.101`, n `668`, weak_sample_signal
- market_context_score -> unknown_forward_1h_return_pct: corr `-0.0859`, n `668`, weak_sample_signal
- market_context_score -> metal_forward_1h_return_pct: corr `-0.0814`, n `668`, weak_sample_signal
- risk_on_score -> crypto_alt_forward_1h_return_pct: corr `0.0813`, n `668`, weak_sample_signal
- flow_alert_score -> crypto_major_forward_1h_return_pct: corr `0.0744`, n `668`, weak_sample_signal
- polymarket_volume_24h -> unknown_forward_1h_return_pct: corr `-0.0738`, n `668`, weak_sample_signal
- polymarket_volume_24h -> equity_forward_1h_return_pct: corr `0.0737`, n `668`, weak_sample_signal
- polymarket_volume_24h -> crypto_major_forward_1h_return_pct: corr `0.0729`, n `668`, weak_sample_signal
