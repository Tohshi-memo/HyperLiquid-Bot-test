# Latest Canary Signals

These are early-warning indicators for cross-market relationships. They are hypotheses to test, not trade signals by themselves.

- Updated: `2026-06-23T15:37:39.138893+00:00`
- Correlation status: `ready`
- Asset price records: `672`
- Minimum samples for correlation: `24`

## Current Signals

- baseline: score `0` - No elevated canary signal. Continue collecting samples.

## Class Returns

- 15m: commodity avg `-0.0744` n `12`; crypto_alt avg `0.0615` n `228`; crypto_major avg `0.1152` n `8`; equity avg `0.086` n `86`; fx avg `-0.0116` n `6`; index avg `-0.0451` n `23`; metal avg `0.1055` n `20`; unknown avg `-0.0045` n `764`
- 1h: commodity avg `0.0198` n `12`; crypto_alt avg `-0.7335` n `228`; crypto_major avg `-0.816` n `8`; equity avg `-0.8057` n `86`; fx avg `-0.0265` n `6`; index avg `-0.1245` n `23`; metal avg `0.1586` n `20`; unknown avg `-0.3654` n `764`
- 4h: commodity avg `-0.2662` n `12`; crypto_alt avg `-0.2467` n `228`; crypto_major avg `-0.5168` n `8`; equity avg `0.4715` n `86`; fx avg `-0.0881` n `6`; index avg `-0.0363` n `23`; metal avg `0.1587` n `20`; unknown avg `-0.3079` n `764`
- 24h: commodity avg `-0.3776` n `12`; crypto_alt avg `-4.2815` n `228`; crypto_major avg `-4.7803` n `8`; equity avg `-3.2898` n `85`; fx avg `-0.1819` n `6`; index avg `-0.925` n `23`; metal avg `-0.993` n `20`; unknown avg `-0.2343` n `604`

## Correlations

- polymarket_volume_24h -> equity_forward_1h_return_pct: corr `0.1324`, n `668`, weak_sample_signal
- flow_alert_score -> index_forward_1h_return_pct: corr `0.1235`, n `668`, weak_sample_signal
- flow_alert_score -> equity_forward_1h_return_pct: corr `0.1193`, n `668`, weak_sample_signal
- polymarket_volume_24h -> index_forward_1h_return_pct: corr `0.1076`, n `668`, weak_sample_signal
- risk_on_score -> metal_forward_1h_return_pct: corr `-0.0854`, n `668`, weak_sample_signal
- market_context_score -> equity_forward_1h_return_pct: corr `0.0791`, n `668`, weak_sample_signal
- risk_on_score -> equity_forward_1h_return_pct: corr `0.0703`, n `668`, weak_sample_signal
- flow_alert_score -> unknown_forward_1h_return_pct: corr `0.0673`, n `668`, weak_sample_signal
- polymarket_volume_24h -> unknown_forward_1h_return_pct: corr `0.0627`, n `668`, weak_sample_signal
- risk_on_score -> crypto_alt_forward_1h_return_pct: corr `-0.0622`, n `668`, weak_sample_signal
