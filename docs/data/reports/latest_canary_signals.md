# Latest Canary Signals

These are early-warning indicators for cross-market relationships. They are hypotheses to test, not trade signals by themselves.

- Updated: `2026-06-23T20:52:32.447685+00:00`
- Correlation status: `ready`
- Asset price records: `672`
- Minimum samples for correlation: `24`

## Current Signals

- baseline: score `0` - No elevated canary signal. Continue collecting samples.

## Class Returns

- 15m: commodity avg `-0.0452` n `12`; crypto_alt avg `-0.154` n `228`; crypto_major avg `-0.1979` n `8`; equity avg `-0.1594` n `86`; fx avg `0.0037` n `6`; index avg `-0.0037` n `23`; metal avg `-0.0117` n `20`; unknown avg `0.114` n `764`
- 1h: commodity avg `-0.0505` n `12`; crypto_alt avg `0.2213` n `228`; crypto_major avg `0.0375` n `8`; equity avg `0.0549` n `86`; fx avg `0.0076` n `6`; index avg `0.0165` n `23`; metal avg `-0.0392` n `20`; unknown avg `0.3809` n `764`
- 4h: commodity avg `0.0682` n `12`; crypto_alt avg `0.2892` n `228`; crypto_major avg `-0.0696` n `8`; equity avg `-0.6898` n `86`; fx avg `0.0038` n `6`; index avg `-0.1017` n `23`; metal avg `-0.2554` n `20`; unknown avg `0.1631` n `756`
- 24h: commodity avg `-0.412` n `12`; crypto_alt avg `-2.7978` n `228`; crypto_major avg `-3.748` n `8`; equity avg `-3.3879` n `86`; fx avg `-0.1837` n `6`; index avg `-0.9348` n `23`; metal avg `-1.231` n `20`; unknown avg `0.5805` n `596`

## Correlations

- polymarket_volume_24h -> equity_forward_1h_return_pct: corr `0.137`, n `668`, weak_sample_signal
- flow_alert_score -> index_forward_1h_return_pct: corr `0.1308`, n `668`, weak_sample_signal
- flow_alert_score -> equity_forward_1h_return_pct: corr `0.1243`, n `668`, weak_sample_signal
- polymarket_volume_24h -> index_forward_1h_return_pct: corr `0.1125`, n `668`, weak_sample_signal
- risk_on_score -> metal_forward_1h_return_pct: corr `-0.0992`, n `668`, weak_sample_signal
- market_context_score -> equity_forward_1h_return_pct: corr `0.0847`, n `668`, weak_sample_signal
- risk_on_score -> equity_forward_1h_return_pct: corr `0.0726`, n `668`, weak_sample_signal
- risk_on_score -> crypto_alt_forward_1h_return_pct: corr `-0.0702`, n `668`, weak_sample_signal
- flow_alert_score -> unknown_forward_1h_return_pct: corr `0.068`, n `668`, weak_sample_signal
- polymarket_volume_24h -> unknown_forward_1h_return_pct: corr `0.0639`, n `668`, weak_sample_signal
