# Latest Canary Signals

These are early-warning indicators for cross-market relationships. They are hypotheses to test, not trade signals by themselves.

- Updated: `2026-09-23T01:27:49.124683+00:00`
- Correlation status: `ready`
- Asset price records: `672`
- Minimum samples for correlation: `24`

## Current Signals

- baseline: score `0` - No elevated canary signal. Continue collecting samples.

## Class Returns

- 15m: commodity avg `0.0091` n `12`; crypto_alt avg `0.0768` n `234`; crypto_major avg `0.012` n `8`; equity avg `0.0667` n `140`; fx avg `-0.0305` n `6`; index avg `0.0026` n `26`; metal avg `-0.0548` n `20`; unknown avg `0.1847` n `945`
- 1h: commodity avg `0.0299` n `12`; crypto_alt avg `-0.2497` n `234`; crypto_major avg `0.0525` n `8`; equity avg `-0.0717` n `140`; fx avg `0.0072` n `6`; index avg `-0.0349` n `26`; metal avg `-0.1681` n `20`; unknown avg `4.4789` n `943`
- 4h: commodity avg `0.115` n `12`; crypto_alt avg `1.3818` n `234`; crypto_major avg `0.9082` n `8`; equity avg `0.0836` n `140`; fx avg `-0.0498` n `6`; index avg `-0.0387` n `26`; metal avg `-0.1065` n `20`; unknown avg `-0.0881` n `936`
- 24h: commodity avg `0.1213` n `12`; crypto_alt avg `3.4081` n `234`; crypto_major avg `1.9721` n `8`; equity avg `0.6679` n `140`; fx avg `-0.2049` n `6`; index avg `0.0414` n `26`; metal avg `0.1274` n `20`; unknown avg `1.0635` n `836`

## Correlations

- news_risk_score -> fx_forward_1h_return_pct: corr `0.1315`, n `668`, weak_sample_signal
- polymarket_volume_24h -> commodity_forward_1h_return_pct: corr `-0.128`, n `668`, weak_sample_signal
- polymarket_volume_24h -> metal_forward_1h_return_pct: corr `-0.1078`, n `668`, weak_sample_signal
- market_context_score -> index_forward_1h_return_pct: corr `0.1057`, n `668`, weak_sample_signal
- risk_on_score -> equity_forward_1h_return_pct: corr `0.1048`, n `668`, weak_sample_signal
- market_context_score -> commodity_forward_1h_return_pct: corr `-0.1005`, n `668`, weak_sample_signal
- flow_alert_score -> metal_forward_1h_return_pct: corr `-0.0997`, n `668`, weak_sample_signal
- risk_on_score -> index_forward_1h_return_pct: corr `0.0989`, n `668`, weak_sample_signal
- market_context_score -> fx_forward_1h_return_pct: corr `-0.0985`, n `668`, weak_sample_signal
- flow_alert_score -> commodity_forward_1h_return_pct: corr `-0.095`, n `668`, weak_sample_signal
