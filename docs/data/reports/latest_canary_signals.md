# Latest Canary Signals

These are early-warning indicators for cross-market relationships. They are hypotheses to test, not trade signals by themselves.

- Updated: `2026-07-10T15:37:30.326297+00:00`
- Correlation status: `ready`
- Asset price records: `672`
- Minimum samples for correlation: `24`

## Current Signals

- baseline: score `0` - No elevated canary signal. Continue collecting samples.

## Class Returns

- 15m: commodity avg `-0.1288` n `12`; crypto_alt avg `0.001` n `229`; crypto_major avg `0.0102` n `8`; equity avg `-0.0183` n `91`; fx avg `-0.0094` n `6`; index avg `0.0227` n `25`; metal avg `0.0441` n `20`; unknown avg `0.0443` n `766`
- 1h: commodity avg `-0.1884` n `12`; crypto_alt avg `0.4377` n `229`; crypto_major avg `0.3092` n `8`; equity avg `0.1601` n `91`; fx avg `-0.0135` n `6`; index avg `0.0849` n `25`; metal avg `0.0908` n `20`; unknown avg `0.0268` n `766`
- 4h: commodity avg `-0.5651` n `12`; crypto_alt avg `-0.3443` n `229`; crypto_major avg `-0.6274` n `8`; equity avg `-0.7748` n `91`; fx avg `-0.0836` n `6`; index avg `0.0687` n `25`; metal avg `0.143` n `20`; unknown avg `-0.1749` n `766`
- 24h: commodity avg `-0.6712` n `12`; crypto_alt avg `0.8909` n `229`; crypto_major avg `1.1367` n `8`; equity avg `-1.0625` n `91`; fx avg `-0.1523` n `6`; index avg `0.056` n `25`; metal avg `-0.1853` n `20`; unknown avg `-0.2131` n `733`

## Correlations

- news_risk_score -> fx_forward_1h_return_pct: corr `0.1257`, n `668`, weak_sample_signal
- polymarket_volume_24h -> metal_forward_1h_return_pct: corr `-0.1066`, n `668`, weak_sample_signal
- flow_alert_score -> index_forward_1h_return_pct: corr `-0.101`, n `668`, weak_sample_signal
- risk_on_score -> metal_forward_1h_return_pct: corr `0.0967`, n `668`, weak_sample_signal
- risk_on_score -> index_forward_1h_return_pct: corr `0.0932`, n `668`, weak_sample_signal
- market_context_score -> fx_forward_1h_return_pct: corr `-0.0919`, n `668`, weak_sample_signal
- polymarket_volume_24h -> index_forward_1h_return_pct: corr `-0.0893`, n `668`, weak_sample_signal
- flow_alert_score -> crypto_alt_forward_1h_return_pct: corr `-0.0757`, n `668`, weak_sample_signal
- polymarket_volume_24h -> crypto_alt_forward_1h_return_pct: corr `-0.0726`, n `668`, weak_sample_signal
- market_context_score -> metal_forward_1h_return_pct: corr `0.0713`, n `668`, weak_sample_signal
