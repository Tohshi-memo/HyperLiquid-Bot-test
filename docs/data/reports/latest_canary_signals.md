# Latest Canary Signals

These are early-warning indicators for cross-market relationships. They are hypotheses to test, not trade signals by themselves.

- Updated: `2026-08-02T10:52:31.949783+00:00`
- Correlation status: `ready`
- Asset price records: `672`
- Minimum samples for correlation: `24`

## Current Signals

- baseline: score `0` - No elevated canary signal. Continue collecting samples.

## Class Returns

- 15m: commodity avg `0.0148` n `12`; crypto_alt avg `-0.0156` n `230`; crypto_major avg `0.028` n `8`; equity avg `0.0183` n `102`; fx avg `-0.0198` n `6`; index avg `0.0016` n `25`; metal avg `-0.0038` n `20`; unknown avg `0.0119` n `782`
- 1h: commodity avg `0.1439` n `12`; crypto_alt avg `-0.2339` n `230`; crypto_major avg `-0.1708` n `8`; equity avg `0.0002` n `102`; fx avg `-0.0177` n `6`; index avg `-0.0035` n `25`; metal avg `-0.0036` n `20`; unknown avg `-0.0675` n `782`
- 4h: commodity avg `0.1057` n `12`; crypto_alt avg `-0.1544` n `230`; crypto_major avg `-0.4005` n `8`; equity avg `0.1459` n `102`; fx avg `-0.042` n `6`; index avg `0.001` n `25`; metal avg `-0.0074` n `20`; unknown avg `-0.124` n `782`
- 24h: commodity avg `-1.0018` n `12`; crypto_alt avg `0.4424` n `230`; crypto_major avg `0.2923` n `8`; equity avg `1.005` n `102`; fx avg `-0.1434` n `6`; index avg `0.2034` n `25`; metal avg `0.244` n `20`; unknown avg `0.2608` n `765`

## Correlations

- news_risk_score -> commodity_forward_1h_return_pct: corr `-0.1275`, n `668`, weak_sample_signal
- polymarket_volume_24h -> commodity_forward_1h_return_pct: corr `0.1212`, n `668`, weak_sample_signal
- flow_alert_score -> commodity_forward_1h_return_pct: corr `0.107`, n `668`, weak_sample_signal
- risk_on_score -> crypto_major_forward_1h_return_pct: corr `-0.093`, n `668`, weak_sample_signal
- polymarket_volume_24h -> crypto_alt_forward_1h_return_pct: corr `-0.0868`, n `668`, weak_sample_signal
- market_context_score -> commodity_forward_1h_return_pct: corr `0.0831`, n `668`, weak_sample_signal
- news_risk_score -> index_forward_1h_return_pct: corr `0.0831`, n `668`, weak_sample_signal
- news_risk_score -> equity_forward_1h_return_pct: corr `0.0748`, n `668`, weak_sample_signal
- risk_on_score -> unknown_forward_1h_return_pct: corr `0.071`, n `668`, weak_sample_signal
- market_context_score -> equity_forward_1h_return_pct: corr `-0.0705`, n `668`, weak_sample_signal
