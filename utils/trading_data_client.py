"""Standard-library client for the Sakura trading-data ingest gateway."""
...
class TradingDataHTTPError(TradingDataError):
    ...
    def __init__(
        ...
        super().__init__(f"trading-data gateway returned HTTP {status}: {message}")
        self.status = status
...
class TradingDataClient:
    ...
    def __init__(
        ...
        *,
        ...
        if parsed.scheme != "https" and not (
            ...
            raise ValueError(
                "base_url must use HTTPS (HTTP is accepted only for localhost tests)."
            )
        if not parsed.netloc or parsed.query or parsed.fragment:
            raise ValueError(
                "base_url must be an origin plus an optional path, without query or fragment."
            )
    ...
    def from_env(
        ...
        *,
        ...
        if secret_file:
            try:
                secret_value = Path(secret_file).read_text(encoding="utf-8").strip()
        ...
        if not base_url or not repo or not secret_value:
            raise TradingDataError(
                "TD_BASE_URL, TD_REPO, and TD_HMAC_SECRET or "
                "TD_HMAC_SECRET_FILE are required."
            )
        return cls(
            base_url,
            repo,
    ...
    def ingest_events(
        ...
        *,
        ...
        """Append a non-empty event batch. Reusing an ID with new content is 409."""
        self._validate_schema_version(schema_version)
        if not events or len(events) > 1_000:
            raise ValueError("events must contain between 1 and 1000 entries.")
        for event in events:
    ...
    def upload_object(
        ...
        *,
        ...
        if observed_start is not None:
            if (
                not _RFC3339_RE.fullmatch(observed_start)
                or not _RFC3339_RE.fullmatch(observed_end or "")
            ):
                raise ValueError("observed timestamps must be RFC3339.")

        path = Path(local_path)
        if not path.is_file() or path.is_symlink():
            raise ValueError("local_path must be a regular, non-symlink file.")
            ...
            try:
                ...
                with urllib.request.urlopen(
                    ...
                    if len(raw) > 1_048_576:
                        raise TradingDataError("Gateway response exceeded 1 MiB.")
    ...
    def _decode_response(raw: bytes) -> dict[str, Any]:
        try:
            ...
            raise TradingDataError("Gateway returned invalid JSON.") from exc
    ...
    def _validate_event_envelope(event: Mapping[str, Any]) -> None:
        ...
        if not isinstance(event, Mapping) or not required.issubset(event):
            raise ValueError("event is missing required envelope fields.")
        if (
        ...
        if (
            ...
            raise ValueError("event payload must be a non-empty object.")
